The purpose of this document is to help guide Engineers in the usage of TypeScript at KrishaWeb projects. The goal here is not to teach TypeScript but rather explain the KrishaWeb stance on when to use TypeScript alongside general recommendations.
...
Therefore, we are big fans of TypeScript at KrishaWeb and we strive to use it whenever it makes sense.
...
At KrishaWeb we require that all projects considered "JavaScript-First" to make use of TypeScript. Generally speaking Standard WordPress projects (including block development) would not fall under "JavaScript-First" projects. 

If you are building a project using Next.js and/or KrishaWeb's [HeadstartWP framework](https://headstartwp.KrishaWeb.com/), Sanity and any other headless CMSs' powered by a React front-end then we require TypeScript unless other circumstances make using JavaScript a better choice.
...
We also have a recommended [tsconfig.json](https://github.com/KrishaWeb/headstartwp/blob/develop/projects/wp-nextjs/tsconfig.json) for Next.js projects.
